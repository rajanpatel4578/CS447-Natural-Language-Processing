...
Original file is located at
    https://colab.research.google.com/drive/1-DT58uf0bqTUVNFhIdiqc-bp9T8OU2Sd
...
class RnnEncoder(nn.Module):
    def __init__(self, src_vocab, embedding_dim, hidden_units):
        ...
        self.gru = nn.GRU(embedding_dim,hidden_units,num_layers=1,batch_first = False, bidirectional = False)
    ...
    def forward(self, x):
        ...
        output,hidden_state = self.gru(embedded.to(device))
...
class RnnDecoder(nn.Module):
    ...
    def compute_attention(self, dec_hs, enc_output):
        ...
        ### TODO ###
        attn_score = self.va(torch.tanh(self.w1(dec_hs.unsqueeze(1))+self.w2(enc_output))).squeeze(-1)
        attn_score = torch.permute(attn_score,(2,1,0))
        ...
        context_vector = torch.sum(attention_weights*torch.transpose(enc_output,1,0),dim=1)
    ...
    def forward(self, x, dec_hs, enc_output):
        ...
        inp = torch.cat((embedded,context_vector.unsqueeze(1)),-1)
        out, dec_hs = self.gru(inp,dec_hs)
        fc_out = self.fc(out)
...
def decode_rnn_model(encoder, decoder, src, max_decode_len, device):
    ...
    enc_out, enc_hs = encoder(src)
    dec_in_hs = enc_hs
    for t in range(1,max_decode_len):
      predictions, dec_in_hs, _ = decoder(dec_input, dec_in_hs, enc_out)
      curr_predictions[:,t] = predictions
      dec_input = predictions.argmax(dim=1)
...
def create_positional_embedding(max_len, embed_dim):
    ...
    demonimator = torch.exp((torch.arange(0,embed_dim,2,dtype = torch.float)*math.log(10000)/embed_dim))
    pe[:,0,0::2] = torch.sin(pos/demonimator)
    pe[:,0,1::2] = torch.cos(pos/demonimator)
    # print('positional embedded: ',pe.size())
    return pe
...
class TransformerEncoder(nn.Module):
    def __init__(self, src_vocab, embedding_dim, num_heads,
        ...
        # Initialize embedding layer
        self.embed = nn.Embedding(src_vocab_size,embedding_dim,padding_idx =0)
        # Dropout layer
        ...
        self.encoder = nn.TransformerEncoder(encoder_layer.to(device),num_layers)
    ...
    def forward(self, x):
        ...
        ### TODO ###
        embedded = self.embed(x)
        # print('word embedded: ',embedded.size())
        pos_embed = embedded.to(device) + self.position_embedding[:embedded.size(0),:,:].to(device)
        pos_embed = self.dropout(pos_embed).squeeze(0)
        # print('pos_embed: ',pos_embed.size())
        src_masks = self.make_src_mask(x)
        # print('src_masks: ',src_masks.size())
        output = self.encoder(pos_embed,src_key_padding_mask = src_masks.squeeze(1))
        # print("len(output.shape)",len(output.shape))
...
class TransformerDecoder(nn.Module):
    def __init__(self, trg_vocab, embedding_dim, num_heads,
        ...
        # Initialize embedding layer
        self.embed = nn.Embedding(trg_vocab_size,embedding_dim,padding_idx=0)
        # Dropout layer
        self.dropout = nn.Dropout(0.5)
        # Initialize a nn.TransformerDecoder model (you'll need to use embedding_dim,
        # num_layers, num_heads, & dim_feedforward here)
        decoder_layer = nn.TransformerDecoderLayer(embedding_dim,num_heads,dim_feedforward)
        self.decoder = nn.TransformerDecoder(decoder_layer.to(device),num_layers)
    ...
    def forward(self, dec_in, enc_out):
        ...
        output = self.embed(dec_in)
        output = output.to(device)+self.position_embedding[:dec_in.size(0),:,:].to(device)
        ...
        output = self.decoder(output.to(device),enc_out.to(device),tgt_mask=trg_mask.to(device))
...
def decode_transformer_model(encoder, decoder, src, max_decode_len, device):
    ...
    ### TODO: Implement decoding algorithm ###
    enc_out = encoder(src.to(device))
    for t in range(1,max_decode_len):
      # dec_input = curr_output[:,:t].long()
      print("dec_input.size(): ",dec_input.size())
      predictions = decoder(dec_input.to(device), enc_out.to(device))
      curr_predictions[:,t,:] = predictions
      print("predictions.size(): ",predictions.size())
      dec_input = predictions.argmax(dim=2)
      print("dec_input.size(): ",dec_input.size())
      print("curr_output.size(): ",curr_output.size())
      curr_output[:,t] = dec_input
