...
Original file is located at
    https://colab.research.google.com/drive/1tZawo6xBi1n62NYUfbLcqu7-ZDVt6s0G
...
class TextDataset(data.Dataset):
        ...
        for line in self.examples:
          for word in line[1]:
            if word in self.word2idx.keys():
              self.textual_ids.append(self.word2idx[word])
            else:
              self.textual_ids.append(self.word2idx[UNK])
          self.textual_ids.append(self.word2idx[END])
    ...
    def get_text(self, idx):
        ...
        ##### TODO #####
        wordList = []
        for word in self.examples[idx][1]:
          if word.lower() in self.word2idx.keys():
            wordList.append(self.word2idx[word.lower()])
          else:
            wordList.append(self.word2idx[UNK])
        wordList.append(self.word2idx[END])
        while len(wordList) < self.max_len:
          wordList.append(self.word2idx[PAD])

        return torch.tensor(wordList[0:self.max_len])
...
class CNN(nn.Module):
    def __init__(self, vocab_size, embed_size, out_channels, filter_heights, stride, dropout, num_classes, pad_idx):
        ...
        self.embed = nn.Embedding(num_embeddings=vocab_size,embedding_dim=embed_size,padding_idx=pad_idx)
        self.convs = nn.ModuleList([nn.Conv2d(in_channels=1, out_channels=out_channels, kernel_size=(filter_height, embed_size), stride=stride, padding=pad_idx) for filter_height in filter_heights])
    ...
    def forward(self, texts):
        ...
        # cat_texts = torch.tensor()
        texts = self.embed(texts)
        # print('shape after embed:',texts.shape)
        texts = torch.unsqueeze(texts,1)
        # print('shape after unsqueeze:',texts.shape)
        for i, layer in enumerate(self.convs):
          # print('value of i:',i)
          # print('value of layer:',layer)
          text = layer(texts)
          # print('shape after passing thru layer:',text.shape)
          text = torch.squeeze(text,dim=-1)
          # print('shape after squeeze:',text.shape)
          text = F.relu(text)
          # print('shape after relu:',text.shape)
          text = torch.amax(text,dim=-1)
          # print('shape post taking max:',text.shape)
          if i==0:
            cat_texts = text
          if i!=0:
            cat_texts = torch.cat((cat_texts,text),-1)
          # print(cat_texts.shape)
        cat_texts = self.dropout(cat_texts)
        # print(cat_texts.shape)
        cat_texts = self.fc1(cat_texts)
        return cat_texts
...
class RNN(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers, bidirectional, dropout, num_classes, pad_idx):
        ...
        self.embed = nn.Embedding(num_embeddings=vocab_size,embedding_dim=embed_size,padding_idx=pad_idx)
        self.rnn = nn.GRU(input_size=embed_size,hidden_size=self.hidden_size,num_layers=self.num_layers, bidirectional=bidirectional,batch_first=True,dropout=dropout)
        self.dropout = nn.Dropout(p=0.5)
    ...
    def forward(self, texts):
        ...
        texts = self.embed(texts)
        # print(texts.shape)
        texts,hn = self.rnn(texts)
        # print(texts.shape)
        # print(hn.shape)
        # texts = torch.squeeze(texts)
        # print("squeeze:",texts.shape)
        if self.bidirectional == True:
          # out = self.rnn(texts,hn)
          cat_texts = torch.cat((texts,texts),1)
        else:
          cat_texts = texts
        # print("cat",cat_texts.shape)
        cat_texts = self.dropout(cat_texts)
        # print("dropout",cat_texts.shape)
        cat_texts = cat_texts[:,-1,:]
        cat_texts = self.fc1(cat_texts)
        
        # print("fc1",cat_texts.shape)
        return cat_texts
